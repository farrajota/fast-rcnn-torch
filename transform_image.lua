...
function M.Vignette(scale)
   return function (input)
      local scalevig = 1 - torch.uniform(0,scale)
      local iW, iH = input:size(3), input:size(2)
      ...
      dis = -dis:sqrt():div(maxDistance):mul(1-scalevig) + 1
...
function M.ColorNormalize(meanstd)
   return function(img)
      ...
      for i=1,img:size(1) do
         if meanstd.mean then img[i]:add(-meanstd.mean[i]) end
         if meanstd.std then img[i]:div(meanstd.std[i]) end
      end
      return img
   end
end

function M.ColorSpace(colorspace)
   local opts = {
      rgb = function(c) return c end,
      bgr = function(input)
         if input:dim() == 3 then
   ...
   local convertFn = opts[colorspace]
   assert(convertFn, 'Undefined input colour space: ' .. colorspace)
   return function(img)
      return convertFn(img)
   end
end
...
function M.FixSize(height, width)
   return function(img)
      local out = img:clone():resize(3, height, width):fill(0)
      out[{{},{1,img:size(2)},{1,img:size(3)}}]:copy(img)
      return out
   end
end
...
-- Scales the smaller edge wrt an upper bound size limit
function M.ScaleLimit(size, max_size, interpolation)
   local interpolation = interpolation or 'bicubic'
   return function(input)
      ...
      local scale = size / im_size_min
      
      -- Prevent the biggest axis from being more than MAX_SIZE
      if math.floor(scale*im_size_max+0.5) > max_size then 
