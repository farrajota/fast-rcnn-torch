...
function Options:parse(opts)
    ...
    _, opt.savedir, opt.manualSeed, opt.GPU, opt.nGPU, opt.nThreads, opt.verbose, opt.progressbar, opt.printConfusion, 
    opt.LR, opt.LRdecay, opt.momentum, opt.weightDecay, opt.optMethod, opt.threshold,
    opt.trainIters, opt.epochStart, opt.schedule, opt.snapshot, opt.optimize, opt.testInter,
    opt.frcnn_scales, opt.frcnn_max_size, opt.frcnn_imgs_per_batch, opt.frcnn_rois_per_img, opt.frcnn_fg_fraction, opt.frcnn_bg_fraction, opt.frcnn_fg_thresh, opt.frcnn_bg_thresh_hi, opt.frcnn_bg_thresh_lo, opt.frcnn_bbox_thresh,
    opt.frcnn_test_scales, opt.frcnn_test_max_size, opt.frcnn_test_nms_thresh, opt.frcnn_test_bbox_voting_nms_thresh, opt.frcnn_test_mode, opt.frcnn_hflip = xlua.unpack(
    {options},
    'Parameters', 
    'Fast-RCNN options',
