...
function options.parse(arg)
  ...
  cmd:text()
  cmd:option('-expID',        'alexnet_teste', 'Experiment ID')
  ...
  cmd:option('-has_bbox_regressor',  "true", 'Enable/disable model\'s box predictor.')
  cmd:text()
  ...
  cmd:text()
  cmd:option('-sampling',           1, 'Use random indexes if > 0, else use index permutation')
  cmd:option('-nEpochs',           20, 'Total number of epochs to run')
  cmd:option('-epochNumber',        1, 'Manual epoch number (useful on restarts)')
  cmd:option('-schedule', "{{14,1e-3,5e-4},{2,1e-4,5e-4}}", 'Optimization schedule. Overrides the previous configs if not empty.')
